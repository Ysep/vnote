...
void MarkdownEditor::updateHeadings(const QVector<vte::md::HeadingInfo> &p_headings) {
  ...
  for (auto const &elem : p_headings) {
    ...
    // The heading may already carry a section number (e.g. "1.2. Title"); reuse
    // it so updateSectionNumber() only rewrites headings whose number drifted.
    QString sectionNumber;
    auto match = vte::MarkdownUtils::matchHeader(block.text());
    if (match.m_matched) {
      sectionNumber = match.m_sequence;
    }
    headings.append(Heading(elem.m_title, elem.m_level, sectionNumber, block.blockNumber(), anchor));
  }
